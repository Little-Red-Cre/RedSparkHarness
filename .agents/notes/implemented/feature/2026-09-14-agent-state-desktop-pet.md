...
Create `dsh-client-ui-pet` as the sole owner of desktop-pet registry entries, preferences, semantic activities, copy, and presentation. Conversation declares one session-optional `conversation.pet` slot and requests either `hero` or `floating` placement. The built-in RedSpark entry supplies normal and chibi four-frame atlases. A loopback browser persists one selection for both placements; a remote browser keeps its selection only in its process. Session state derives idle, thinking, waiting, error, and completion; other plugins may report coding, tool work, or sleeping through `ctx.pet` without a runtime import between feature packages. The Electron carrier accepts an application-served absolute PNG path without traversal or query, or a bounded embedded PNG frame, and hides even when a removed provider leaves a stale atlas in a hidden update.
