...
flowchart LR
  ...
  svc_authorizationController["ctx.authorizationController<br/>Host account-authorization Remote controller"]
...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | 将部署选中的账号流程投影为脱敏元数据和浏览器持有的通知/问题流；凭据仍由 Host provider 持有。 |
