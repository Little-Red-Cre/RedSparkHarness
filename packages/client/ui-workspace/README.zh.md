...
会话行菜单提供带确认的“删除会话”。该操作通过持久化归档集合移除左侧条目，不清除会话记录、不停止运行中的任务，也不取消定时计划。操作失败时保留条目并显示错误。
...
- **没有永久删除 Session 与取消归档控件**：历史记录链接可以打开已归档会话，不恢复左侧条目；普通导航仍会清除已归档的选择。删除 Workspace 注册记录不会删除 Session。
