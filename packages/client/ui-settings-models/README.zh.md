...
`dsh-client-ui-settings-models` 是 dsh Web 客户端的 Models 设置页面：用户可以配置 API 密钥（以只写方式存入 profile 的凭据引用之下）、运行提供方自有的账号登录流程、编辑每个提供方的模型列表，并手工声明自定义 pi-ai 路由；页面以提供方行展示，一次只展开一张编辑卡片。该页面把提供方目录、设置文档与凭据描述合并为一个共享快照，因此行的状态在三个方面始终一致。它还会带首次运行的用户走两个有序弹窗——版本化内测声明，以及按条件显示的官方 DeepSeek 凭据步骤。
...
交付的 Web profile 将 OpenAI Codex 放在**订阅模型**下。其已配置路由和休眠目录项均不出现在 API 模型列表及提供方候选项中，包括首次运行的 API 密钥设置。其他适配器注册的登录流程保持隐藏，除非 Host 显式启用。

在 OpenAI Codex 下点击**登录并添加模型**。登录反馈与浏览器链接显示在该账号下方，进度通知保留最近一次有效链接。授权流程和凭据记录归 Host 所有。登录成功后自动为缺失的 `openai-codex` 提供方创建不含 API-key 引用的 profile。已经登录的账号在打开页面时自动执行同一检查，无需再次登录或点击启用按钮。写入携带设置版本并保留已有 profile；API-key 覆盖、只读设置或注册失败会显示错误及“重试添加模型”。关闭页面会取消尚未开始写入的准备工作，已提交的设置保持不变。在会话的模型选择器中选择 Codex 模型后即可发送消息，已有会话选项不会被自动修改。Agent Loop 仍由 RedSpark 执行。真实账号授权与订阅推理仍需人工验证。
...
**运行时不变式：** 不发布伴生入口。该 section 只通过生成的 Remote 读取脱敏状态、执行显式配置写入与转发当前浏览器的授权回答；它不持有提供方凭据，也不接管 Agent Loop。
