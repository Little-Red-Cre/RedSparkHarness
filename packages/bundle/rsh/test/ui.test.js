...
describe('filterResumeSessions', () => {
  const sessions = [
    { id: 'session-a', title: 'Fix renderer', cwd: 'D:\\RedSparkHarness', createdAt: 10, updatedAt: 20 },
    { id: 'session-b', title: 'Plan release', cwd: 'D:\\Other', createdAt: 30, updatedAt: 15 },
    { id: 'session-c', title: 'Review RSH resume', cwd: 'D:\\RedSparkHarness', createdAt: 25, updatedAt: 35 },
  ];

  test('filters to the current directory and sorts by update time by default', () => {
    assert.deepEqual(
      filterResumeSessions(sessions, { cwd: 'D:\\RedSparkHarness', scope: 'cwd', search: '', sort: 'updated' }).map(session => session.id),
      ['session-c', 'session-a'],
    );
  });

  test('searches all session display fields and supports creation-time sorting', () => {
    assert.deepEqual(
      filterResumeSessions(sessions, { cwd: 'D:\\RedSparkHarness', scope: 'all', search: 'release', sort: 'created' }).map(session => session.id),
      ['session-b'],
    );
  });
});

describe('resumePageCapacity', () => {
  test('accounts for the three rendered lines occupied by each session row', () => {
    assert.equal(resumePageCapacity(24), 5);
    assert.equal(resumePageCapacity(40), 11);
  });

  test('keeps a usable one-row browser in a very small terminal', () => {
    assert.equal(resumePageCapacity(1), 1);
  });
});
