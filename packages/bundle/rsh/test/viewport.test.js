...
const BASE = { rows: 24, width: 80, busy: false, todos: [] };
...
describe('estimateItemLines', () => {
  ...
  test('assistant reasoning is always counted because the TUI always displays it', () => {
    const item = { kind: 'assistant', text: 'answer', reasoning: 'some private thought' };
    const withReasoning = estimateItemLines(item, 80);
    const withoutReasoning = estimateItemLines({ ...item, reasoning: '' }, 80);
    assert.ok(withReasoning > withoutReasoning, 'visible reasoning should estimate taller');
  });
...
describe('computeViewport', () => {
  ...
  test('optional todos argument defaults safely', () => {
