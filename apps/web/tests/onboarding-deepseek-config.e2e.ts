...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    ...
    const credentialStep = page.getByRole('dialog', { name: '为 RedSpark Harness 配置模型' })
    ...
    expect(await page.getByRole('dialog', { name: '为 RedSpark Harness 配置模型' }).count()).toBe(0)
    ...
    await page.addInitScript(() => {
      ...
      setInterval(() => {
        if (document.querySelector(
          ...
          + '[role="dialog"][aria-label="为 RedSpark Harness 配置模型"]',
        ) !== null) {
