...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('synchronizes the pet style with the welcome hero and persists it after reload', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-pet-preferences'))
    const sprite = page.locator('[data-placement="hero"] [role="img"]')
    await sprite.waitFor({ timeout: 10_000 })
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.getByRole('button', { name: '通用设置', exact: true }).click()
    await dialog.getByRole('button', { name: 'Q版', exact: true }).click()
    await expect.poll(() => sprite.getAttribute('style')).toContain('kitsune-chibi-sprites.png')
    await page.keyboard.press('Escape')
    ...
    await page.getByRole('button', { name: '设置', exact: true }).click()
    await dialog.getByRole('button', { name: '通用设置', exact: true }).click()
    expect(await dialog.getByRole('button', { name: 'Q版', exact: true }).getAttribute('aria-pressed')).toBe('true')
    await dialog.getByRole('switch', { name: '显示桌宠', exact: true }).click()
    await expect.poll(() => sprite.count()).toBe(0)
    await dialog.getByRole('switch', { name: '显示桌宠', exact: true }).click()
    await dialog.getByRole('button', { name: '正常版', exact: true }).click()
    await expect.poll(() => sprite.getAttribute('style')).toContain('kitsune-sprites.png')
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  })

  it('imports a custom pet, restores it after reload, and removes it without losing the built-in choices', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-pet-import'))
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.getByRole('button', { name: '通用设置', exact: true }).click()
    const upload = dialog.getByLabel('导入桌宠 PNG', { exact: true })
    await upload.setInputFiles({ name: 'invalid.png', mimeType: 'image/png', buffer: Buffer.from('invalid') })
    await dialog.getByRole('alert').filter({ hasText: '导入或保存失败' }).waitFor()
    const buffer = await readFile(new URL('../../desktop/renderer/redspark.png', import.meta.url))
    await upload.setInputFiles({ name: 'My fox.png', mimeType: 'image/png', buffer })
    await expect.poll(() => dialog.getByRole('button', { name: 'My fox', exact: true }).getAttribute('aria-pressed')).toBe('true')
    const sprite = page.locator('[data-placement="hero"] [role="img"]')
    await expect.poll(() => sprite.getAttribute('style')).toContain('data:image/png;base64,')
    await page.keyboard.press('Escape')
    const warningStart = tripwire.warnings.length
    await page.reload({ waitUntil: 'load' })
    await page.waitForSelector('[class*="frame"]', { timeout: 30_000 })
    ...
    await page.getByRole('button', { name: '设置', exact: true }).click()
    await dialog.getByRole('button', { name: '通用设置', exact: true }).click()
    await dialog.getByRole('button', { name: '移除当前自定义桌宠', exact: true }).click()
    await expect.poll(() => dialog.getByRole('button', { name: 'My fox', exact: true }).count()).toBe(0)
    await expect.poll(() => sprite.getAttribute('style')).toContain('/brand/kitsune-sprites.png')
    expect(await dialog.getByRole('button', { name: 'Q版', exact: true }).count()).toBe(1)
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  })
